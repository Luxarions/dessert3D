...
export function setQuatFromProperEuler(q: any, a: number, b: number, c: number, order: string) {
  const cos = Math.cos;
  const sin = Math.sin;
  ...
  switch (order) {
    ...
    case 'ZYZ':
      ...
      break;
    default:
      warn('MathUtils: .setQuatFromProperEuler() encountered an unknown order: ' + order);
  }
}

export function denormalize(value: number, array: any): number {
  switch (array.constructor) {
    ...
    default: throw new Error('LXRN.MathUtils: Invalid component type.');
  }
}

export function normalize(value: number, array: any): number {
  switch (array.constructor) {
    case Float32Array: return value;
...
export const MathUtils = {
  DEG2RAD, RAD2DEG,
  generateUUID, clamp, euclideanModulo, mapLinear, inverseLerp, lerp, damp,
  pingpong, smoothstep, smootherstep, randInt, randFloat, randFloatSpread,
  seededRandom, degToRad, radToDeg, isPowerOfTwo, ceilPowerOfTwo, floorPowerOfTwo,
  setQuatFromProperEuler, normalize, denormalize
};
